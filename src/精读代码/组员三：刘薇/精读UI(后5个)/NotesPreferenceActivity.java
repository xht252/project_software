...
public class NotesPreferenceActivity extends PreferenceActivity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        getActionBar().setDisplayHomeAsUpEnabled(true);//给左上角的图标加上一个返回的图标
        addPreferencesFromResource(R.xml.preferences);//添加xml来源，并显示xml
    ...
    protected void onResume() {
        ...
        if (mHasAddedAccount) {
            //若用户新加了账户，则自动设置同步账户
            Account[] accounts = getGoogleAccounts();//获取谷歌同步账户
            ...
            public boolean onPreferenceClick(Preference preference) {
                ...
                if (!GTaskSyncService.isSyncing()) {
                    if (TextUtils.isEmpty(defaultAccount)) {
                        ...
                        //若是第一次建立账户，则显示选择账户提示框
                        showSelectAccountAlertDialog();
                    } else {
                        // if the account has already been set, we need to promp
                        // user about the risk
                        //若是已经建立，则显示修改对话框并进行修改操作
    ...
    private void showChangeAccountConfirmAlertDialog() {
        ...
        dialogBuilder.setItems(menuItemArray, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {//按键功能，由which来决定
                if (which == 0) {
                    //进入账户选择对话框
                    showSelectAccountAlertDialog();
                } else if (which == 1) {
